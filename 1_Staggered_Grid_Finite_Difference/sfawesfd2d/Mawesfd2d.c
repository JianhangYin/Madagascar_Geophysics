...
void step_forward(float **p, float **vz, float **vx, float **k, float **rho, float *coef){
	...
	for(ix=N;ix<nxpad-N-1;ix++){
		for(iz=N;iz<nzpad-N;iz++){
			...
			for(iop=1;iop<N+1;iop++){
				tmpx=tmpx+(p[ix+iop][iz]-p[ix-iop+1][iz])*coef[iop];
			}
	...
	for(ix=N;ix<nxpad-N;ix++){
		for(iz=N;iz<nzpad-N-1;iz++){
			tmpx=0;
			for(iop=1;iop<N+1;iop++){
				tmpx=tmpx+(p[ix][iz+iop]-p[ix][iz-iop+1])*coef[iop];
			}
	...
	for(ix=N;ix<nxpad-N;ix++){
		for(iz=N;iz<nzpad-N;iz++){
			...
			p[ix][iz]=p[ix][iz]+(tmp1*(1.0/dx)+tmp2*(1.0/dz))*dt*k[ix][iz];
		}
	}
}
...
int main(int argc, char* argv[]){
	...
	for(it=0; it<nt; it++)
	{
		...
		p[sx][sz]+=wlt[it];
		step_forward(p,vz, vx, k, rho,coef);
		apply_sponge(p, bndr);
